...
@Repository
public class NoRankedAnimeJDBC implements NoRankedAnimeDao {
...
*/
    ...
    public List<NoRankedAnime>  selectAllQuerry(List<Map<String, Object>>  result) {
    	try {
    		
    	
    		List<NoRankedAnime> animeList = new ArrayList<>();
    		for(Map<String,Object>map:result) {
    			NoRankedAnime anime = new NoRankedAnime();
    		...
    		List<NoRankedAnime> animeList = new ArrayList<>();
    		animeList.add(new NoRankedAnime());
    		return animeList;
    	}
    }
    @Override
    public List<NoRankedAnime> selectAll(Integer year) {
    	try {
    		String sql = "select *, rank() over (order by score desc) as ranking " +
   	             ...
   	             "where year = ? and (score is null or  score =0 ) " +
   	             "order by ranking";
