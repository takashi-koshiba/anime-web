...
document.addEventListener("DOMContentLoaded",function(){
	...
	let initImgCount=ImgCount*2+2;//最初に表示する画像の数  ×2+2以上にしてください
	let imgRootUrl;
	let imgData;
	...
	function buttonEvent(direction,rank_scroll,rankWidth,rankObj,anime_childW,imgData,imgRootUrl){
		moveElements(direction,rank_scroll,rankWidth,rankObj,anime_childW,imgData,imgRootUrl);
		//margin(rank_scroll,anime_childW);
	...
	function addElement(index,imgData,rank_scroll,imgRootUrl,rank_animeTemplate,imgMargin){
		...
		if(animeLen>index&&index>=0){
			...
			//console.dir(rank_scroll.children[0].children.length+":"+index);
			//rank_scroll.children[0].children[0].style.marginLeft =imgMargin+"px";
			//rank_scroll.children[0].children[0].style.marginRight=imgMargin+"px";
	...
	function moveElements(direction,rank_scroll,rankWidth,rankObj,anime_childW,imgData,imgRootUrl){
		
		//console.dir(rankObj.pos);
		...
		for (let i=0;i<elements.length;i++){
			...
			if(direction==1&&rankObj.index!=0){//右に移動
				...
				//elements[i].style.transform=`translate(${rankObj.pos+(direction*rankWidth)}px,0)`;
				rank_scroll.scrollTo({
					left:(rankObj.pos*-1)+rankWidth,
					behavior:'smooth'
				});
		...
		//rankObj.pos=count!=0?rankObj.pos+rankWidth*direction:rankObj.pos;
		//rankObj.index+=count!=0?(Math.floor(rankWidth/anime_childW)*-direction):0;
		//console.dir(rankObj.pos);
	}
	...
	function processRankElems(rankObj,scrollPos,imgData,rank_scroll,imgRootUrl,rank_animeTemplate,imgMargin,anime_childW){
		...
		if(rankObj.index-scrollPos<0){
			...
			for(let i=imgIndex;i+abs> imgIndex;i--){
				...
				//rankObj.index + init で現在の一番最後の座標が出るのでは？
				//あとは座標+表示されている個数　のindexを削除すればよいのでは？
		}
	...
	function delElem(imgIndex,rank_scroll){
		//rank_scroll.children[0].children;
	...
	function centering(rank_scroll,anime_childW,rankObj,initImgCount,imgData,imgRootUrl,rank_animeTemplate){
		...
		//スクロールしたときに位置を調節する
		//ボタンを押したときは問題がないが指でスクロールしたときに位置がずれるため
